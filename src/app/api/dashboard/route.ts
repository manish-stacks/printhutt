...
export async function GET() {
  try {
    ...
    revenue.forEach(({ _id, totalRevenue }) => {
      const index = weeklyRevenue.labels.indexOf(daysMap[_id.dayOfWeek]);
...
export async function GET() {
  try {
    ...
    const orderCount = revenue.reduce((total, order) => total + order.orderCount, 0);
    const totalRevenue = revenue.reduce((total, order) => total + order.totalRevenue, 0);
